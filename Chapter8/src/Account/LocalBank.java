...
public class LocalBank 
{
	...
	private void update()
	{
		...
		if (selectedAccount != null) //If there is a selected account
		{
			...
			if (selectedAccount.getClass() == new BusinessAcct("",0,"","","","","","",0,"").getClass()) //If the selected account is a BusinessAcct and not a PersonalAcct
			{
				idLabel.setEnabled(true);
				idLabel.setText("Selected account ID: " + selectedAccount.id + " (Business account)"); //Add (Business account) to the end of the ID label
			}
			else if (selectedAccount.getClass() == new PersonalAcct("",0,"","","","","","",0).getClass()) //If the selected account is a PersonalAcct and not a BusinessAcct
			{
				idLabel.setEnabled(true);
				idLabel.setText("Selected account ID: " + selectedAccount.id + " (Personal account)"); //Add (Personal account) to the end of the ID label
			}
			else //If it matches neither subclass (something has gone horribly wrong)
			{
				idLabel.setEnabled(true);
				idLabel.setText("Selected account ID: " + selectedAccount.id + " (Unknown account)"); //Just a filler version so it doesn't all break
			}
